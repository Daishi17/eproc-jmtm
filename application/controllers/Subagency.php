...
class Subagency extends CI_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->load->model('Sub_agency/Sub_agency_model');
	...
	public function index()
	{
		...
		$this->load->view('sub_agency/index');
		$this->load->view('template_panitia/footer');
		$this->load->view('sub_agency/ajax');
	}

	public function tambah_sub_agency()
	{
		$data['getdivisi'] = $this->Rup_model->get_devisi();
		$data['pegawai'] = $this->Sub_agency_model->getPegawai();
		...
		$this->load->view('template_panitia/header');
		$this->load->view('template/navlogin');
		$this->load->view('sub_agency/tambah_subagency', $data);
		$this->load->view('template_panitia/footer');
		$this->load->view('sub_agency/ajax');
	}
	public function edit_sub_agency($id_pegawai)
	{
		$data['getdivisi'] = $this->Rup_model->get_devisi();
		$data['pegawai'] = $this->Sub_agency_model->getPegawai();
		$data['area'] = $this->Area_model->getArea();
		$data['kabupaten'] = $this->Wilayah_model->getAllKabupaten();
		$data['provinsi'] = $this->Wilayah_model->getProvinsi();
		$data['agency'] = $this->Sub_agency_model->getAllByIdAgency($id_pegawai);
		$this->load->view('template_panitia/header');
		$this->load->view('template/navlogin');
		$this->load->view('sub_agency/edit_subagency', $data);
		$this->load->view('template_panitia/footer');
		$this->load->view('sub_agency/ajax');
	}
	public function getdata()
	{
		$resultss = $this->Sub_agency_model->getdatatable();
		$data = [];
		$no = $_POST['start'];
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->Sub_agency_model->count_all_data(),
			"recordsFiltered" => $this->Sub_agency_model->count_filtered_data(),
			"data" => $data
		);
	...
	public function add()
	{
		...
		$this->form_validation->set_rules('username', 'Username Pegawai', 'required|trim|is_unique[tbl_pegawai.username]', ['required' => 'NIP NIP Pegawai Wajib Diisi!', 'is_unique' => 'Username Sudah Terdaftar']);
		$this->form_validation->set_rules('nama_pegawai', 'Nama Pegawai', 'required|trim', ['required' => 'Nama Pegawai Wajib Diisi!']);
		$this->form_validation->set_rules('password', 'Password Pegawai', 'required|trim', ['required' => 'Password Pegawai Wajib Diisi!']);
		$this->form_validation->set_rules('alamat', 'Alamat Pegawai', 'required|trim', ['required' => 'Alamat Pegawai Wajib Diisi!']);
		...
		$this->form_validation->set_rules('no_sk', 'Nomor Sk Pegawai', 'required|trim', ['required' => 'Nomor Sk Pegawai Wajib Diisi!']);
		$this->form_validation->set_rules('id_unit_kerja', 'Area', 'required|trim', ['required' => 'Area Wajib Diisi!']);
		if ($this->form_validation->run() == false) {
			...
			$this->Sub_agency_model->create($data);
	...
	public function update()
	{
		...
		$this->form_validation->set_rules('email', 'Email Pegawai', 'required|trim|valid_email|xss_clean' . $is_unique_email, ['required' => 'Email Wajib Diisi!', 'valid_email' => 'Email Tidak Valid', 'is_unique' => 'Email Pegawai Sudah Terdaftar']);
		...
		if ($this->form_validation->run() == false) {
			...
			$this->Sub_agency_model->update(array('id_pegawai' => $this->input->post('id_pegawai')), $data);
			$this->output->set_content_type('application/json')->set_output(json_encode('berhasil'));
		}
	}
	...
	{
		$data = $this->Sub_agency_model->getByid($id_pegawai);
		$output = [
			'get_sub_agency' => $data
		];
		$this->output->set_content_type('application/json')->set_output(json_encode($output));
	...
	{
		$this->Sub_agency_model->delete($id_pegawai);
	...
	public function gantipassword($id_pegawai)
	{
		...
		if ($this->form_validation->run() == false) {
			$data['pegawai'] = $this->Sub_agency_model->getById2($id_pegawai);
			$this->load->view('template_panitia/header');
			$this->load->view('template/navlogin');
			$this->load->view('sub_agency/gantipassword', $data);
			$this->load->view('template_panitia/footer');
			$this->load->view('sub_agency/ajax', $data);
		} else {
			...
			// var_dump($data);
			// die;
			$this->Sub_agency_model->updatepassword($data, $id_pegawai);
			...
			redirect('subagency/edit_sub_agency/' . $id_pegawai);
		}
	}
}
