...
class Vendor extends CI_Controller
{
	...
	public function vendor_perorangan()
	{
		$this->load->view('template_vendor/header');
		$this->load->view('template_vendor/navbar');
		$this->load->view('vendor/vendor_perorangan');
		$this->load->view('template_vendor/footer');
		$this->load->view('vendor/ajax');
	}
	...
	public function get_vendor()
	{
		...
		$data = [];
		$no = $_POST['start'];
		$session = $this->session->userdata('id_role');
		foreach ($result as $vendor) {
			//$cek_dokumen = $this->Vendor_model->cek_sudah_tervaldiasi_dokumen($vendor->id_vendor);
			$row = array();
			$row[] = ++$no;
			$row[] = $vendor->username_vendor;
			$row[] = $vendor->email_vendor;
			if ($vendor->status_ditolak == 1) {
				$row[] = '<div class="badge badge-danger">Vendor Ditolak!</div>';
			} else {
				$row[] = '<div class="badge badge-info">Vendor Belum Diterima!</div>';
			}
			if ($session == 2) {
				if ($vendor->status_ditolak == 1) {
					$row[] = ' <a href="javascript:;" class="btn btn-success btn-sm" onClick="byid(' . "'" . $vendor->id_vendor . "','approve_vendor'" . ')"><i class="far fa-thumbs-up"></i> Terima</a> <button disabled class="btn btn-danger btn-sm" onClick="byid(' . "'" . $vendor->id_vendor . "','tolak_vendor'" . ')"><i class="far fa-thumbs-up"></i> Tolak</button>';
				} else {
					$row[] = ' <a href="javascript:;" class="btn btn-success btn-sm" onClick="byid(' . "'" . $vendor->id_vendor . "','approve_vendor'" . ')"><i class="far fa-thumbs-up"></i> Terima</a> <a href="javascript:;" class="btn btn-danger btn-sm" onClick="byid(' . "'" . $vendor->id_vendor . "','tolak_vendor'" . ')"><i class="far fa-thumbs-up"></i> Tolak</a>';
				}
			} else {
				if ($vendor->status_ditolak == 1) {
					$row[] = '<a href="' . base_url('vendor/detail/' . $vendor->id_vendor) . '" class="btn btn-info btn-sm" onClick="byid(' . "'" . $vendor->id_vendor . "','detail_vendor'" . ')"><i class="fa fa-search"></i> Detail</a> <a href="javascript:;" class="btn btn-success btn-sm" onClick="byid(' . "'" . $vendor->id_vendor . "','approve_vendor'" . ')"><i class="far fa-thumbs-up"></i> Terima</a> <button disabled class="btn btn-danger btn-sm" onClick="byid(' . "'" . $vendor->id_vendor . "','tolak_vendor'" . ')"><i class="far fa-thumbs-up"></i> Tolak</button>';
				} else {
	...
	public function get_vendor_perorangan()
	{
		$result = $this->Vendor_model->getdatatable2_perorangan();
		// var_dump($result);
		// die;
		$data = [];
		$no = $_POST['start'];
		foreach ($result as $vendor) {
			...
			$row[] = ++$no;
			$cek_dokumen = $this->Vendor_model->cek_sudah_tervaldiasi_dokumen($vendor->id_vendor);
			// if ($vendor->notifikasi_pergantian_izin_usaha == 1) {
			// 	$row[] = $vendor->username_vendor . ' <div class="badge badge-danger">!</div>';
			// } else {
			// 	$row[] = $vendor->username_vendor;
			// }
			$row[] = $vendor->nama_usaha;
			if (!$vendor->id_jenis_pengadaan) {
				$row[] = 'Belum Memilih Jenis Pengadaan';
			} else {
				$row[] = $vendor->nama_jenis_pengadaan;
			}
			$row[] = $vendor->kualifikasi_usaha;
			$row[] = $vendor->email_vendor;
			if ($cek_dokumen) {
				$row[] = '<div class="badge badge-warning">Dokumen Sudah Tervalidasi</div>';
			} else {
				$row[] = '<div class="badge badge-danger">Dokumen Belum Tervalidasi</div>';
			}
			if ($vendor->status_validasi_dokumen == 1) {
				$row[] = '<div class="badge badge-warning">Undangan Sudah Terkirim</div>';
			} else {
				$row[] = '<div class="badge badge-danger">Undangan Belum Terkirim</div>';
			}
			if ($this->session->userdata('id_role') == 2) {
				$row[] = '<a href="' . base_url('vendor/detail_vendor_aktif/' . $vendor->id_vendor) . '" class="btn btn-block btn-info btn-sm"><i class="fa fa-search"></i> Detail</a>';
			} else {
				$row[] = '<a href="javascript:;" onclick="sudah_dibaca_notifikasi_validator(' . $vendor->id_vendor . ')" class="btn btn-block btn-info btn-sm"><i class="fa fa-search"></i> Detail</a> <a href="javascript:;" class="btn btn-block btn-danger btn-sm" onClick="byid(' . "'" . $vendor->id_vendor . "','daftar_hitam'" . ')"><i class="fa fa-ban"></i> Blacklist</a>';
			}
			$data[] = $row;
			// href="' . base_url('vendor/detail_vendor_aktif/' . $vendor->id_vendor) . '" 
		}
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->Vendor_model->count_all_data2_perorangan(),
			"recordsFiltered" => $this->Vendor_model->count_filtered_data2_perorangan(),
			"data" => $data
		);
		$this->output->set_content_type('application/json')->set_output(json_encode($output));
	}
	...
	public function approve_vendor($id_vendor)
	{
		...
		// Isi email
		$this->email->message("Selamat Pendaftaran Penyedia Anda Sudah Diterima Silahkan Lakukan Login Dengan Password Dan Username Yang Anda Isi");
	...
	public function detail_vendor_aktif($id_vendor)
	{
		$data['vendor'] = $this->Vendor_model->getById($id_vendor);
		$data['cek_verifikasi'] = $this->Vendor_model->cek_Verifikasi($id_vendor);
		$data['persyaratan_vms'] = $this->Vendor_model->get_all_persyaratan();
		$data['persyaratan_vms_pemborongan'] = $this->Vendor_model->get_all_persyaratan_pemborongan();
		$data['persyaratan_vms_perorangan'] = $this->Vendor_model->get_all_persyaratan_perorangan();
		$data['all_verifikasi'] = $this->Vendor_model->all_verifikasi($id_vendor);
		$data['cek_dokumen_tervalidasi'] = $this->Vendor_model->cek_sudah_tervaldiasi_dokumen($id_vendor);
		$data['get_jenis_pengadaan'] = $this->Vendor_model->get_jenis_pengadaan_result();
		// new update
		$data['sbu'] = $this->Vendor_model->get_nama_sbu($id_vendor);


		$data['get_izin_usaha']  = $this->Vendor_model->get_izin_usaha($id_vendor);
		$data['get_akta'] = $this->Vendor_model->get_akta($id_vendor);
		$data['get_pemilik'] = $this->Vendor_model->get_pemilik($id_vendor);
		$data['get_pengurus'] = $this->Vendor_model->get_pengurus($id_vendor);
		$data['get_tenaga_ahli'] = $this->Vendor_model->get_tenaga_ahli($id_vendor);
		$data['get_peralatan'] = $this->Vendor_model->get_peralatan($id_vendor);
		$data['get_pengalaman'] = $this->Vendor_model->get_pengalaman($id_vendor);
		$data['get_pajak'] = $this->Vendor_model->get_pajak($id_vendor);
		$data['get_neraca_keuangan'] = $this->Vendor_model->get_neraca_keuangan($id_vendor);
		$data['get_nib'] = $this->Vendor_model->get_nib($id_vendor);
		$data['get_tdp'] = $this->Vendor_model->get_tdp($id_vendor);
		$data['get_siup'] = $this->Vendor_model->get_siup($id_vendor);
		$data['get_npwp'] = $this->Vendor_model->get_npwp($id_vendor);
		$data['get_skpkp'] = $this->Vendor_model->get_skpkp($id_vendor);
		$data['get_sppt'] = $this->Vendor_model->get_sppt($id_vendor);
		$data['get_siujk'] = $this->Vendor_model->get_siujk($id_vendor);
		$data['get_ktp'] = $this->Vendor_model->get_ktp($id_vendor);
		$data['get_sbu'] = $this->Vendor_model->get_sbu($id_vendor);
		$data['get_domisili'] = $this->Vendor_model->get_domisili($id_vendor);
		$data['get_bagan'] = $this->Vendor_model->get_bagan($id_vendor);
		$data['get_ketenaga_kerjaan'] = $this->Vendor_model->get_ketenaga_kerjaan($id_vendor);
		$data['get_ketenaga_kesehatan'] = $this->Vendor_model->get_ketenaga_kesehatan($id_vendor);

		// 30-juni-2022
		$data['get_sbu_master'] = $this->Vendor_model->get_sbu_master($id_vendor);
		$this->load->view('template_vendor/header');
		$this->load->view('template_vendor/navbar');
		$this->load->view('vendor/detail_vendor_aktif', $data);
		$this->load->view('template_vendor/footer');
		$this->load->view('vendor/ajax_vms');
	}
	...
	public function vendor_pendaftar()
	{
		$this->load->view('template_vendor/header');
		$this->load->view('template_vendor/navbar');
	...
	public function get_vendor_blacklist()
	{
		...
		foreach ($result as $vendor) {
			...
			$row[] = ++$no;
			$row[] = $vendor->username_vendor;
			$row[] = $vendor->alasan_daftar_hitam;
			$row[] = '<a href="https://eproc.jmtm.co.id/file_daftar_hitam/' . $vendor->file_daftar_hitam . '">' . '<img width="40px" src=' . base_url('assets/img/pdf.png') . ' >' . '</a>';
	...
	// lupa password
	public function lupa_password_vendor()
	{
		$this->load->view('template_vendor/header');
		$this->load->view('template_vendor/navbar');
		$this->load->view('vendor/lupa_password_vendor');
		$this->load->view('template_vendor/footer');
		$this->load->view('vendor/ajax');
	}
	...
	public function data_get_pemilik($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
				...
				$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_pemilik(' . "'" . $angga->id_pemilik . "','edit_pemilik'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-sm btn-block" onClick="byid_pemilik(' . "'" . $angga->id_pemilik . "','hapus_pemilik'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
			}
	...
	public function data_get_pengurus($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
				...
				$row[] = '<button disabled href="javascript:;" class="btn btn-block btn-warning btn-sm" onClick="byid_pengurus(' . "'" . $angga->id_pengurus . "','edit_pengurus'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-sm" onClick="byid_pengurus(' . "'" . $angga->id_pengurus . "','hapus_pengurus'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
			}
			$data[] = $row;
		}
		$output = array(
	...
	public function data_get_tenaga_ahli($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			$row[] = '<a target="_blank" href="https://vms.jmtm.co.id/file_bukti_tenaga_ahli/' . $angga->file_bukti_pendidikan . '">' . '<img width="40px" src=' . base_url('assets/img/pdf.png') . ' >' . '</a>';
			$row[] = $angga->sertifikasi_tenaga_ahli;
			$row[] = $angga->kualifikasi_keahlian_tenaga_ahli;
			$row[] = $angga->tanggal_masa_berlaku_sertifikasi;
			$row[] = '<a target="_blank" href="https://vms.jmtm.co.id/file_bukti_tenaga_ahli/' . $angga->file_bukti_sertifikat_tenaga_ahli . '">' . '<img width="40px" src=' . base_url('assets/img/pdf.png') . ' >' . '</a>';
	...
	public function data_get_peralatan($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
				...
				$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_peralatan(' . "'" . $angga->id_peralatan . "','edit_peralatan'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_peralatan(' . "'" . $angga->id_peralatan . "','hapus_peralatan'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
			}
	...
	public function data_get_pengalaman($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->tanggal_kontrak == 0000 - 00 - 00) {
				$row[] = '-';
			} else {
				$row[] = date('d-m-Y', strtotime($angga->tanggal_kontrak));
			}
			if ($angga->selesai_kontrak == '-') {
				$row[] = '-';
			} else {
				$row[] = date('d-m-Y', strtotime($angga->selesai_kontrak));
			}
			if ($angga->status_tidak_punya == 1) {
				$row[] = '-';
			} else {
				$row[] = '<a target="_blank" href="https://vms.jmtm.co.id/bukti_pengalaman/' . $angga->referensi_kontrak . '">' . '<img width="40px" src=' . base_url('assets/img/pdf.png') . ' >' . '</a>';
			}

			if ($angga->status_tidak_punya == 1) {
				$row[] = '-';
			} else {
				$row[] = '<a target="_blank" href="https://vms.jmtm.co.id/bukti_pengalaman/' . $angga->bukti_pengalaman . '">' . '<img width="40px" src=' . base_url('assets/img/pdf.png') . ' >' . '</a>';
			}

			$row[] = $angga->date_created2;
			if ($angga->status_tidak_punya == 1) {
				$row[] = '<div class="badge badge-danger">Tidak Punya Pengalaman</div>';
			} else {
				if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
					...
					$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_pengalaman(' . "'" . $angga->id_pengalaman . "','edit_pengalaman'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_pengalaman(' . "'" . $angga->id_pengalaman . "','hapus_pengalaman'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
				}
			}
	...
	public function data_get_pajak($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
				...
				$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_pajak(' . "'" . $angga->id_pajak . "','edit_pajak'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_pajak(' . "'" . $angga->id_pajak . "','hapus_pajak'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
			}
	...
	public function data_get_keuangan($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
				...
				$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_keuangan(' . "'" . $angga->id_keuangan . "','edit_keuangan'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_keuangan(' . "'" . $angga->id_keuangan . "','hapus_keuangan'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
			}
	...
	public function data_get_nib($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
				...
				$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_nib(' . "'" . $angga->id_nib . "','edit_nib'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_nib(' . "'" . $angga->id_nib . "','hapus_nib'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
			}
	...
	public function data_get_tdp($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
				...
				$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_tdp(' . "'" . $angga->id_tdp . "','edit_tdp'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_tdp(' . "'" . $angga->id_tdp . "','hapus_tdp'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
			}
			$data[] = $row;
			// <a href="javascript:;" class="btn btn-success btn-block btn-sm" onClick="byid_nib(' . "'" . $angga->id_nib . "','edit_nib'" . ')"><i class="fas fa-edit"></i> Edit</a>
		}
	...
	public function data_get_siup($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_tidak_punya == 1) {
				$row[] = '<div class="badge badge-danger">Tidak Punya SIUP</div>';
			} else {
				if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
					...
					$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_siup(' . "'" . $angga->id_siup . "','edit_siup'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_siup(' . "'" . $angga->id_siup . "','hapus_siup'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
				}
			}
	...
	public function data_get_npwp($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
				...
				$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_npwp(' . "'" . $angga->id_npwp . "','edit_npwp'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_npwp(' . "'" . $angga->id_npwp . "','hapus_npwp'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
			}
			$data[] = $row;
			// <a href="javascript:;" class="btn btn-success btn-block btn-sm" onClick="byid_nib(' . "'" . $angga->id_nib . "','edit_nib'" . ')"><i class="fas fa-edit"></i> Edit</a>
		}
	...
	public function data_get_skpkp($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
				...
				$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_skpkp(' . "'" . $angga->id_skpkp . "','edit_skpkp'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_skpkp(' . "'" . $angga->id_skpkp . "','hapus_skpkp'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
			}
			$data[] = $row;
			// <a href="javascript:;" class="btn btn-success btn-block btn-sm" onClick="byid_nib(' . "'" . $angga->id_nib . "','edit_nib'" . ')"><i class="fas fa-edit"></i> Edit</a>
		}
	...
	public function data_get_sppt($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
				...
				$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_sppt(' . "'" . $angga->id_sppt . "','edit_sppt'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_sppt(' . "'" . $angga->id_sppt . "','hapus_sppt'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
			}
			$data[] = $row;
			// <a href="javascript:;" class="btn btn-success btn-block btn-sm" onClick="byid_nib(' . "'" . $angga->id_nib . "','edit_nib'" . ')"><i class="fas fa-edit"></i> Edit</a>
		}
	...
	public function data_get_siujk($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_tidak_punya == 1) {
				$row[] = '<div class="badge badge-danger">Tidak Punya SIUJK</div>';
			} else {
				if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
					...
					$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_siujk(' . "'" . $angga->id_siujk . "','edit_siujk'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_siujk(' . "'" . $angga->id_siujk . "','hapus_siujk'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
				}
			}
	...
	public function data_get_ktp($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
				...
				$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_ktp(' . "'" . $angga->id_ktp . "','edit_ktp'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_ktp(' . "'" . $angga->id_ktp . "','hapus_ktp'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
			}
			$data[] = $row;
			// <a href="javascript:;" class="btn btn-success btn-block btn-sm" onClick="byid_nib(' . "'" . $angga->id_nib . "','edit_nib'" . ')"><i class="fas fa-edit"></i> Edit</a>
		}
	...
	public function data_get_sbu($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			$row = array();
			$row[] = ++$no;
			$row[] = $angga->kode_sbu . ' | ' . $angga->nama_sbu;
			$row[] = $angga->kualifikasi_usaha_sbu;
			if ($angga->masa_berlaku_seumur_hidup == 1) {
				$row[] = 'SEUMUR HIDUP';
			} else if ($angga->masa_berlaku_seumur_hidup == NULL || $angga->masa_berlaku_seumur_hidup == 0) {
				$row[] = $angga->masa_berlaku_awal . ' S/D ' . $angga->masa_berlaku_akhir;
			...
			if ($this->session->userdata('id_role') == 1) {
				if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
					$row[] = '<a href="javascript:;" class="btn btn-success btn-block btn-sm" onClick="byid_sbu(' . "'" . $angga->id_sbu_vendor . "','edit_sbu'" . ')"><i class="fas fa-check"></i> Sesuai</a> 
					<a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_sbu(' . "'" . $angga->id_sbu_vendor . "','hapus_sbu'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a> 
					<a href="javascript:;" class="btn btn-info btn-block btn-sm" onClick="byid_sbu(' . "'" . $angga->id_sbu_vendor . "','ubah_sbu'" . ')"><i class="fas fa-edit"></i> Ubah</a>';
				} else if ($angga->status_sesuai == 1) {
					$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_sbu(' . "'" . $angga->id_sbu_vendor . "','edit_sbu'" . ')"><i class="fas fa-check"></i> Sesuai</button> 
					<a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_sbu(' . "'" . $angga->id_sbu_vendor . "','hapus_sbu'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>
					<a href="javascript:;" class="btn btn-info btn-block btn-sm" onClick="byid_sbu(' . "'" . $angga->id_sbu_vendor . "','ubah_sbu'" . ')"><i class="fas fa-edit"></i> Ubah</a>';
				}
			} else {
	...
	public function approve_sbu()
	{
		...
		$this->Vendor_model->update_sbu(array('id_sbu_vendor' => $this->input->post('id_sbu')), $data);
		$this->output->set_content_type('application/json')->set_output(json_encode($data));
	}
	...
	public function data_get_domisili($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
				...
				$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_domisili(' . "'" . $angga->id_domisili . "','edit_domisili'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_domisili(' . "'" . $angga->id_domisili . "','hapus_domisili'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
			}
			$data[] = $row;
			// <a href="javascript:;" class="btn btn-success btn-block btn-sm" onClick="byid_nib(' . "'" . $angga->id_nib . "','edit_nib'" . ')"><i class="fas fa-edit"></i> Edit</a>
		}
	...
	public function data_get_bagan($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
				...
				$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_bagan(' . "'" . $angga->id_bagan . "','edit_bagan'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_bagan(' . "'" . $angga->id_bagan . "','hapus_bagan'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
			}
			$data[] = $row;
			// <a href="javascript:;" class="btn btn-success btn-block btn-sm" onClick="byid_nib(' . "'" . $angga->id_nib . "','edit_nib'" . ')"><i class="fas fa-edit"></i> Edit</a>
		}
	...
	public function data_get_bpjs_ketenagakerjaan($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_tidak_punya == 1) {
				$row[] = '<div class="badge badge-danger">Tidak Punya BPJS KETENAGAKERJAAN</div>';
			} else {
				if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
					...
					$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_bpjs_ketenagakerjaan(' . "'" . $angga->id_bpjs_ketenagakerjaan . "','edit_bpjs_ketenagakerjaan'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_bpjs_ketenagakerjaan(' . "'" . $angga->id_bpjs_ketenagakerjaan . "','hapus_bpjs_ketenagakerjaan'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
				}
			}
	...
	public function data_get_bpjs_kesehatan($id_vendor)
	{
		...
		foreach ($resultss as $angga) {
			...
			if ($angga->status_tidak_punya == 1) {
				$row[] = '<div class="badge badge-danger">Tidak Punya BPJS KESEHATAN</div>';
			} else {
				if ($angga->status_sesuai == 0 || $angga->status_sesuai == NULL) {
					...
					$row[] = '<button disabled href="javascript:;" class="btn btn-warning btn-block btn-sm" onClick="byid_bpjs_kesehatan(' . "'" . $angga->id_bpjs_kesehatan . "','edit_bpjs_kesehatan'" . ')"><i class="fas fa-check"></i> Sesuai</button> <a href="javascript:;" class="btn btn-danger btn-block btn-sm" onClick="byid_bpjs_kesehatan(' . "'" . $angga->id_bpjs_kesehatan . "','hapus_bpjs_kesehatan'" . ')">  <i class="fas fa-times"></i> Tidak Sesuai</a>';
				}
			}
	...
	{
		//status vendor aktif
		$data['vendor_aktif'] = $this->Vendor_model->status_vendor_aktif();
		$data['vendor_pending'] = $this->Vendor_model->status_vendor_pending();
		$data['vendor_blacklist'] = $this->Vendor_model->status_vendor_blacklist();

		// 		$data['status_vendor_tervalidasi'] = $this->Vendor_model->status_vendor_tervalidasi();
		// 		$data['status_vendor_belum_tervalidasi'] = $this->Vendor_model->status_vendor_belum_tervalidasi();
		...
		$this->load->view('template_vendor/footer');
		$this->load->view('vendor/ajax_grafik', $data);
	}

	public function kirim_email_pembuktian()
	{
		...
		$data = [
			'status_validasi_dokumen' => 1
		];

		//update status validasi dokumen di tbl_vendor
		$this->Vendor_model->update_status_dokumen($id_vendor, $data);
		...
		// Email penerima
		$this->email->to($email_usaha); // Ganti dengan email tujuan
		...
		<label class="col-md-2 col-form-label" style="text-alig">Hari<span style="color:red;">*</span></label>
		<div class="col-md-3">
		<b>' . $hari . '</b>
		</div>
		</div>
		<div class="form-group row" style="margin-top: -20px;">
		<label class="col-md-2 col-form-label" style="text-alig">Tanggal<span style="color:red;">*</span></label>
		<div class="col-md-3">
		<b>' . $tanggal . '</b>
		</div>
		</div>
		';
		// Isi email
		$this->email->message("Dokumen anda sudah tervalidasi silahkan lakukan pembuktian dokumen pada hari $hari, $tanggal");

		$this->email->send();
	}

	public function ubah_kualifikasi_usaha($id_vendor)
	{
		$where = [
			'id_vendor' => $id_vendor
		];
		...
		$this->Vendor_model->update_identitas($data, $where);
		$config = array(
			'protocol' => 'smtp',
		...
		$kirim_email = $this->Vendor_model->get_vendor($id_vendor);
		...
		// Isi email
		$this->email->message("Kepada Penyedia $kirim_email->username_vendor Kualifikasi Usaha Anda Telah Diganti Silahkan Login Untuk Memeriksanya https://vms.jmtm.co.id/auth");

		$this->email->send();
		redirect('vendor/detail_vendor_aktif/' . $id_vendor);
	}


	public function notifikasi_validator()
	{
		$data = $this->Vendor_model->total_notif_validator();
		$output = [
			'dataku' => $data
		];
		$this->output->set_content_type('application/json')->set_output(json_encode($output));
	...
	public function notifikasi_validator_sudah_dibaca($id_vendor)
	{
		$data = [
			'notifikasi_pergantian_izin_usaha' => 0
		];
		$id_vendor2 = [
			'id_vendor' => $id_vendor
		];
		$db = $this->Vendor_model->update_sudah_dibaca_validator($id_vendor2, $data);
		$output = [
			'dataku' => $db
		];
		$this->output->set_content_type('application/json')->set_output(json_encode($output));
	...
	public function ganti_password_vendor($id_vendor)
	{
		...
		if ($password_vendor) {
			...
			$this->Vendor_model->update_sudah_dibaca_validator($where, $data);
			$this->output->set_content_type('application/json')->set_output(json_encode('success'));
		} else {
	...
	public function menolak_vendor($id_vendor)
	{
		$data = [
			'status_ditolak' => 1
		];
		...
		// Isi email
		$this->email->message("Mohon Maaf Pendaftaran Vendor Management System Anda Di Tolak Dengan Username : $kirim_email->username_vendor !!!");

		$this->email->send();
	}

	public function dokumen_tervalidasi()
	{
		$email_usaha = $this->input->post('email_usaha2');
		$username_vendor = $this->input->post('username_vendor2');
		$hari = $this->input->post('hari');
		$tanggal = $this->input->post('tanggal');
		$id_vendor = $this->input->post('id_vendor');
		// $data = [
		...
		// Isi email
		$this->email->message("Yth. Penyedia $username_vendor Anda Sudah Tidak Perlu Melakukan Pembuktian Dikarenakan Anda Sudah Melakukan Pembuktian Sebelumnya dan Sudah Bisa Mengikuti Tender Yang Ada Pada PT. Jasamarga Toll Road Maintenance");
	...
	// new update
	public function vendor_kontraktor()
	{
		$data['sbu'] = $this->db->query("SELECT * FROM tbl_sbu")->result_array();
		$this->load->view('template_vendor/header');
		$this->load->view('template_vendor/navbar');
		$this->load->view('vendor/vendor_kontraktor', $data);
		$this->load->view('template_vendor/footer');
		$this->load->view('vendor/ajax');
	}

	public function get_vendor_kontraktor()
	{
		$result = $this->Vendor_model->getdatatable2_kontraktor();
		// var_dump($result);
		// die;
		$data = [];
		$no = $_POST['start'];
		foreach ($result as $vendor) {
			...
			$row[] = ++$no;
			$cek_dokumen = $this->Vendor_model->cek_sudah_tervaldiasi_dokumen($vendor->id_vendor);
			$sbu = $this->Vendor_model->get_nama_sbu($vendor->id_vendor);
			...
			$row[] = $vendor->kualifikasi_usaha;
			$row[] = '<a href="javascript:;" onClick="byid(' . "'" . $vendor->id_vendor . "','sbu'" . ')" class="btn btn-info btn-block btn-sm"><i class="fa fa-search"></i> Lihat SBU</a>';
			$row[] = $vendor->email_vendor;
			if ($cek_dokumen) {
				$row[] = '<div class="badge badge-warning">Dokumen Sudah Tervalidasi</div>';
			} else {
			...
			if ($this->session->userdata('id_role') == 2) {
				$row[] = '<a href="' . base_url('vendor/detail_vendor_aktif/' . $vendor->id_vendor) . '" class="btn btn-info btn-block btn-sm"><i class="fa fa-search"></i> Detail</a>';
			} else {
				$row[] = '<a href="javascript:;" onclick="sudah_dibaca_notifikasi_validator(' . $vendor->id_vendor . ')" class="btn  btn-block btn-info btn-sm"><i class="fa fa-search"></i> Detail</a> <a href="javascript:;" class="btn btn-danger btn-sm" onClick="byid(' . "'" . $vendor->id_vendor . "','daftar_hitam'" . ')"><i class="fa fa-ban"></i> Blacklist</a>';
			}
			$data[] = $row;
			// href="' . base_url('vendor/detail_vendor_aktif/' . $vendor->id_vendor) . '" 
		}
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->Vendor_model->count_all_data2_kontraktor(),
			"recordsFiltered" => $this->Vendor_model->count_filtered_data_kontraktor(),
			"data" => $data
		);
		$this->output->set_content_type('application/json')->set_output(json_encode($output));
	}
	...
	public function vendor_nonkontraktor()
	{
		$this->load->view('template_vendor/header');
		$this->load->view('template_vendor/navbar');
		$this->load->view('vendor/vendor_nonkontraktor');
		$this->load->view('template_vendor/footer');
		$this->load->view('vendor/ajax');
	}

	public function get_vendor_nonkontraktor()
	{
		$result = $this->Vendor_model->getdatatable2_nonkontraktor();
		// var_dump($result);
		// die;
		$data = [];
		$no = $_POST['start'];
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->Vendor_model->count_all_data2_nonkontraktor(),
			"recordsFiltered" => $this->Vendor_model->count_filtered_data_nonkontraktor(),
			"data" => $data
		);
		$this->output->set_content_type('application/json')->set_output(json_encode($output));
	}
	...
	{
		$uploadpath = '/home/nginx/domains/vms.jmtm.co.id/public/file_bukti_sbu';
		$config['upload_path'] = $uploadpath;
		$config['allowed_types'] = 'pdf';
		$config['max_size'] = 0;

		$where = [
			'id_sbu_vendor' => $this->input->post('id_sbu_vendor')
		];


		$this->load->library('upload', $config);

		if ($this->upload->do_upload('file_bukti_sbu_vendor')) {
			...
			$upload = [
				...
				// 'masa_berlaku_seumur_hidup' => $this->input->post('masa_berlaku_seumur_hidup'),
				'id_sbu_master' => $this->input->post('id_sbu'),
				'kualifikasi_usaha_sbu' => $this->input->post('kualifikasi_usaha_sbu'),
				'file_bukti_sbu_vendor' => $fileData['file_name']
			];
			// var_dump($upload);
			// die;
			$this->Vendor_model->update_sbu($where, $upload);
			$this->output->set_content_type('application/json')->set_output(json_encode($upload));
		} else {
			// $fileData = $this->upload->data();
	...
	// 1 juli 2022
	public function ganti_jenis_pengadaan()
	{
		$where = [
			'id_vendor' => $this->input->post('id_vendor_update_jenis_pengadaan'),
		];
		$data = [
			'id_jenis_pengadaan' => $this->input->post('id_jenis_pengadaan'),
		];
		$this->Vendor_model->update_ke_identitas($where, $data);
		$this->output->set_content_type('application/json')->set_output(json_encode('success'));
	}

	// 27 mei 2023
	public function tambah_daftar_hitam()
	{
		...
		$config['upload_path'] = 'file_daftar_hitam';
		$config['allowed_types'] = 'pdf';
		$config['max_size'] = 0;
		$this->load->library('upload', $config);
		if ($this->upload->do_upload('file_daftar_hitam')) {

			$fileData = $this->upload->data();

			$data = [
				'username_vendor' => $nama_usaha,
				'password_vendor' => password_hash('123', PASSWORD_DEFAULT),
				'alasan_daftar_hitam' => $alasan_daftar_hitam,
				'file_daftar_hitam' => $fileData['file_name'],
				'status_aktive_vendor' => 0,
				'masa_berlaku_daftar_hitam_mulai' => date('d-m-Y H:i'),
				'masa_berlaku_daftar_hitam_selesai' => date('d-m-Y H:i', strtotime("+2 year")),
				'status_daftar_hitam_vendor' => 1
			...
			$data2 = [
				...
				'id_vendor' => $package_id
			];
			$this->db->insert('tbl_vendor_identitas_prusahaan', $data2);
		}
		$this->output->set_content_type('application/json')->set_output(json_encode('success'));
	}
}
