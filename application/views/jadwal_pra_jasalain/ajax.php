<script>
	...
	$(document).ready(function() {
		tabledata.DataTable({
			...
			"order": [],
			"ajax": {
				"url": "<?= base_url('jadwal_tender/getdata9') ?>",
				"type": "POST"
			},
			"columnDefs": [{
				"target": [-1],
	...
	function save() {
		if (saveData == 'tambah') {
			url = "<?= base_url('jadwal_tender/add9'); ?>"
		} else {
			url = "<?= base_url('jadwal_tender/update9'); ?>"
		}
		$.ajax({
			method: "POST",
			url: url,
	...
	function byid(id, type) {
		...
		$.ajax({
			type: "GET",
			url: "<?= base_url('jadwal_tender/byid9/'); ?>" + id,
			dataType: "JSON",
			success: function(response) {
				if (type == 'edit') {
	...
	function deleteData(id) {
		$.ajax({
			type: "POST",
			url: "<?= base_url('jadwal_tender/delete9/') ?>" + id,
			dataType: "JSON",
			success: function(response) {
				if (response == 'success') {
					relodTable();
