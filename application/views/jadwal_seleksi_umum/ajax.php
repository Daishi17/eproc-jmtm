<script>
	...
	$(document).ready(function() {
		tabledata.DataTable({
			...
			"order": [],
			"ajax": {
				"url": "<?= base_url('jadwal_tender/getdata6') ?>",
				"type": "POST"
			},
			"columnDefs": [{
				"target": [-1],
	...
	function save() {
		if (saveData == 'tambah') {
			url = "<?= base_url('jadwal_tender/add6'); ?>"
		} else {
			url = "<?= base_url('jadwal_tender/update6'); ?>"
		}
		$.ajax({
			method: "POST",
			url: url,
	...
	function byid(id, type) {
		...
		$.ajax({
			type: "GET",
			url: "<?= base_url('jadwal_tender/byid6/'); ?>" + id,
			dataType: "JSON",
			success: function(response) {
				if (type == 'edit') {
	...
	function deleteData(id) {
		$.ajax({
			type: "POST",
			url: "<?= base_url('jadwal_tender/delete6/') ?>" + id,
			dataType: "JSON",
			success: function(response) {
				if (response == 'success') {
					relodTable();
