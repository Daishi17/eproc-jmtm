<script>
	...
	$(document).ready(function() {
		tabledata.DataTable({
			...
			"order": [],
			"ajax": {
				"url": "<?= base_url('nib/getdata2/') ?>" + id_nib,
				"type": "POST"
			},
			"columnDefs": [{
				"target": [-1],
	...
	function save() {
		if (saveData == 'tambah') {
			url = "<?= base_url('nib/add2'); ?>"
		} else {
			url = "<?= base_url('nib/update2'); ?>"
		}
		$.ajax({
			method: "POST",
			url: url,
	...
	function byid(id, type) {
		...
		$.ajax({
			type: "GET",
			url: "<?= base_url('nib/byid2/'); ?>" + id,
			dataType: "JSON",
			success: function(response) {
				if (type == 'detail') {
					location.replace('<?= base_url('nib/detailkategori2/') ?>' + response.id_sub_kategori)
				}
				if (type == 'edit') {
					modaltitle.text('Ubah Data');
					$('[name="id_nib"]').val(response.id_nib);
					$('[name="id_sub_kategori"]').val(response.id_sub_kategori);
					$('[name="sub_kategori_nib"]').val(response.sub_kategori_nib);
					modal.modal('show');
				}
				if (type == 'hapus') {
					deleteQuestion(response.id_sub_kategori, response.sub_kategori_nib);
				}
			}
		})
	}
