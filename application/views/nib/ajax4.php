<script>
	...
	$(document).ready(function() {
		tabledata.DataTable({
			...
			"order": [],
			"ajax": {
				"url": "<?= base_url('nib/getdata4/') ?>" + id_sub_kategori,
				"type": "POST"
			},
			"columnDefs": [{
				"target": [-1],
	...
	function save() {
		if (saveData == 'tambah') {
			url = "<?= base_url('nib/add4'); ?>"
		} else {
			url = "<?= base_url('nib/update4'); ?>"
		}
		$.ajax({
			method: "POST",
			url: url,
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (type == 'edit') {
					...
					$('[name="sub_kategori_nib3"]').val(response.sub_kategori_nib3);
					modal.modal('show');
				}
				if (type == 'hapus') {
					deleteQuestion(response.id_sub_kategori3, response.sub_kategori_nib3);
				}
			}
		})
	}
