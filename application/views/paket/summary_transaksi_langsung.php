...
<body style="font-size: 13px;">
	<div class="container">
		...
		<input type="hidden" value="<?= $angga['id_paket'] ?>" id="id_paket">
		<br>
				...
				<div class="content tab-content">
					...
					<table class="table table-bordered">
						...
						<?php foreach ($dokumen_pengadaan as $key => $value) { ?>
							<tr>
								<td><?= $value['file_dokumen_pengadaan_transaksi_langsung'] ?></td>
							...
							<tr>
								<td style="width: 30%;">
									...
									<?php   } else if ($jadwal['tanggal_selesai'] >= date('d-m-Y H:i') || $jadwal['tanggal_mulai'] == date('d-m-Y H:i')) { ?>
										<b class="text-danger"> <?= $jadwal['nama_jadwal_transaksi_langsung_detail'] ?> <small class="text text-danger">Tahap Sedang Berlangsung</small></b>
									<?php   } else { ?>

										<b class="text-success"> <?= $jadwal['nama_jadwal_transaksi_langsung_detail'] ?> <small class="text text-success">Tahap Sudah Selesai <i class="fa fa-check"></i> </small></b>
									<?php   } ?>
								</td>
	...
	<script>
		$(document).ready(function() {
			...
			function pesan2() {
				...
				$.ajax({
					type: "post",
					url: "<?= base_url() ?>paket/ngeload_chat_transaksi_langsung/" + id_paket2,
					data: {
