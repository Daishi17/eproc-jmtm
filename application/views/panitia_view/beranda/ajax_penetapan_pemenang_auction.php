<script>
	...
	var tbl_penetapan_pemenang_auction = $('#tbl_penetapan_pemenang_auction');
	var formData = $('#formData');
	var modal_evaluasititle = $('#modal_evaluasiTitle');
	var btnsave = $('#btnSave');
	$(document).ready(function() {
		var id_paket_evaluasi = $('#id_paket_evaluasi').val();
		tbl_penetapan_pemenang_auction.DataTable({
			"responsive": true,
			"autoWidth": false,
			"processing": true,
			"serverSide": true,
			"order": [],
			"ajax": {
				"url": "<?= base_url('panitiajmtm/beranda/tbl_penetapan_pemenang_auction/') ?>" + id_paket_evaluasi,
				"type": "POST"
			},
			"columnDefs": [{
				"target": [-1],
	...
	function relodTable() {
		tbl_penetapan_pemenang_auction.DataTable().ajax.reload();
	}
