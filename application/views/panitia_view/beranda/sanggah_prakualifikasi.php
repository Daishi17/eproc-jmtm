...
<div id="main" class="container">
	...
	<ul class="nav nav-tabs">
		...
		<?php	} else if ($get_tahap_dokumen_sangahan_akhir['tanggal_selesai_jadwal']  >= date('d-m-Y H:i') || $get_tahap_dokumen_sangahan_akhir['tanggal_mulai_jadwal']  == date('d-m-Y H:i')) { ?>
			<li class="nav-item">
				<a class="nav-link bg-primary text-white ml-1" href="<?= base_url('panitiajmtm/beranda/sanggah_prakualifikasi/' . $paket['id_paket']) ?>" aria-controls="home" aria-selected="true">Sanggahan Prakualifikasi</a>
			</li>
		<?php	} else { ?>
			<li class="nav-item">
				<a class="nav-link bg-primary text-white ml-1" href="<?= base_url('panitiajmtm/beranda/sanggah_prakualifikasi/' . $paket['id_paket']) ?>" aria-controls="home" aria-selected="true">Sanggahan Prakualifikasi</a>
		...
		<?php	} else if ($get_tahap_prakualfiikasi_satu_file_penetapan['tanggal_selesai_jadwal']  >= date('d-m-Y H:i') || $get_tahap_prakualfiikasi_satu_file_penetapan['tanggal_mulai_jadwal']  == date('d-m-Y H:i')) { ?>
			<li class="nav-item">
				<a class="nav-link bg-info text-white ml-1" href="<?= base_url('panitiajmtm/beranda/negosiasi/' . $paket['id_paket']) ?>" aria-controls="home" aria-selected="true">Negosiasi</a>
			</li>
		<?php	} else { ?>
			<li class="nav-item">
				<a class="nav-link bg-info text-white ml-1" href="<?= base_url('panitiajmtm/beranda/negosiasi/' . $paket['id_paket']) ?>" aria-controls="home" aria-selected="true">Negosiasi</a>
				...
				foreach ($get_file_sanggah as $key => $value) { ?>
					...
					foreach ($get_file_sanggah as $key => $value) { ?>
						<tr>
							...
							<td><a target="_blank" href="https://vms.jmtm.co.id/file_dokumen_sanggahan/<?= $value['file_dokumen_sanggahan'] ?>"><img src="<?= base_url('assets/img/file-icon.png') ?>" style="width: 50px;" alt=""></a></td>
							<td>
								<?php if ($value['balasan_sanggahan_panitia'] == NULL) { ?>

								<?php } else { ?>
									<a target="_blank" href="<?= base_url('file_dokumen_sanggahan_prakualifikasi/' . $value['balasan_sanggahan_panitia']) ?>"><img src="<?= base_url('assets/img/file-icon.png') ?>" style="width: 50px;" alt=""></a>
								<?php } ?>
							</td>
							<td><?= $value['keterangan_panitia'] ?></td>
							<td><a href="javascript:;" data-toggle="modal" data-target="#balas_sanggahan_akhir<?= $value['id_dokumen_sanggahan_prakualifikasi'] ?>" class="btn btn-sm btn-primary">Balas</a></td>
						</tr>
					<?php    } ?>
				<?php	} ?>
...
<?php foreach ($get_file_sanggah as $key => $value) { ?> ?>
	<div class="modal fade" id="balas_sanggahan_akhir<?= $value['id_dokumen_sanggahan_prakualifikasi'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header bg-info text-white">
					<h5 class="modal-title" id="modalTitle">Balas Sanggahan Prakualifikasi</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<form action="<?= base_url('panitiajmtm/beranda/balas_sanggahan_prakualifikasi/' . $paket['id_paket']) ?>" method="POST" enctype="multipart/form-data">
					<div class="modal-body">
						<input type="hidden" name="id_dokumen_sanggahan_prakualifikasi" value="<?= $value['id_dokumen_sanggahan_prakualifikasi'] ?>">
